...
pub fn derive_root_signing_key(seed: &str) -> Result<SigningKey, JsError> {
    let stripped = seed.strip_prefix("0x").unwrap_or(seed);
    let bytes = hex::decode(stripped).map_err(map_js_error!("Failed to decode hex seed: {}"))?;
    ...
    SigningKey::from_bytes((&key_bytes).into()).map_err(map_js_error!(
        "failed to derive signing key from signature: {}"
    ))
}
...
pub fn derive_sk_root_signing_key(
    ...
    hex_to_signing_key(&sk_root_hex)
}

/// Derive the sk_root scalar from the root key and nonce
pub fn derive_sk_root_scalars(seed: &str, nonce: &Scalar) -> Result<NonNativeScalar<2>, JsError> {
    let root_key = derive_root_signing_key(seed)?;

    let sk_root = derive_sk_root(&root_key, Some(nonce))?;
    let sk_root_biguint = BigUint::from_bytes_be(&sk_root.to_bytes_be());
    let sk_root_nonnative = NonNativeScalar::try_from(&sk_root_biguint)?;
    Ok(sk_root_nonnative)
}

/// Construct sk_root by signing the concat of the root key message and the nonce
pub fn derive_sk_root(root_key: &SigningKey, nonce: Option<&Scalar>) -> Result<Scalar, JsError> {
...
pub fn derive_symmetric_key(root_key: &SigningKey) -> Result<HmacKey, JsError> {
...
pub fn derive_wallet_keychain(root_key: &SigningKey) -> Result<KeyChain, JsError> {
    // Generate the root key

    let sk_root = root_key.try_into()?;
    let pk_root = root_key.verifying_key().try_into()?;
...
pub fn derive_blinder_seed(root_key: &SigningKey) -> Result<Scalar, JsError> {
...
pub fn derive_share_seed(root_key: &SigningKey) -> Result<Scalar, JsError> {
...
pub fn derive_wallet_id(root_key: &SigningKey) -> Result<WalletIdentifier, JsError> {
    let bytes = get_extended_sig_bytes(WALLET_ID_MESSAGE, root_key)?;
    WalletIdentifier::from_slice(&bytes[..WALLET_ID_BYTES])
        .map_err(map_js_error!("failed to derive wallet ID from key: {}"))
}
...
) -> Result<(ApiWallet, Scalar, Scalar), JsError> {
    // Derive the seeds and keychain
    let wallet_id = derive_wallet_id(signing_key)?;
    let blinder_seed = derive_blinder_seed(signing_key)?;
    let share_seed = derive_share_seed(signing_key)?;
    let keychain = derive_wallet_keychain(signing_key)?;

    let wallet = Wallet::new_empty_wallet(wallet_id, blinder_seed, share_seed, keychain)?;
    let api_wallet = wallet.try_into()?;
    Ok((api_wallet, blinder_seed, share_seed))
}

/// Get a `Scalar` from a signature on a message
fn derive_scalar(msg: &[u8], key: &SigningKey) -> Result<Scalar, JsError> {
...
fn get_sig_bytes(msg: &[u8], key: &SigningKey) -> Result<[u8; KECCAK_HASH_BYTES], JsError> {
...
fn get_extended_sig_bytes(msg: &[u8], key: &SigningKey) -> Result<[u8; EXTENDED_BYTES], JsError> {
...
pub async fn derive_wallet_id_external(
...
) -> Result<WalletIdentifier, JsError> {
    ...
    let wallet_id = WalletIdentifier::from_slice(&bytes[..WALLET_ID_BYTES])
        .map_err(map_js_error!("Failed to create wallet ID: {}"))?;

    Ok(wallet_id)
}

pub async fn derive_blinder_seed_external(sign_message: &Function) -> Result<Scalar, JsError> {
    // Sign the blinder seed message and convert to a scalar
    derive_scalar_external(BLINDER_STREAM_SEED_MESSAGE, sign_message).await
}

/// Construct the share seed for the wallet
pub async fn derive_share_seed_external(sign_message: &Function) -> Result<Scalar, JsError> {
    // Sign the share seed message and convert to a scalar
    derive_scalar_external(SHARE_STREAM_SEED_MESSAGE, sign_message).await
}

pub async fn derive_sk_match_external(sign_message: &Function) -> Result<Scalar, JsError> {
    derive_scalar_external(MATCH_KEY_MESSAGE, sign_message).await
}

pub async fn derive_symmetric_key_external(sign_message: &Function) -> Result<HmacKey, JsError> {
...
/// Get a `Scalar` from a signature on a message
async fn derive_scalar_external(msg: &[u8], sign_message: &Function) -> Result<Scalar, JsError> {
...
async fn get_extended_sig_bytes_external(
    msg: &[u8],
    sign_message: &Function,
) -> Result<[u8; EXTENDED_BYTES], JsError> {
...
async fn get_sig_bytes_external(
    msg: &[u8],
    sign_message: &Function,
) -> Result<[u8; KECCAK_HASH_BYTES], JsError> {
    let digest = keccak256(msg);
