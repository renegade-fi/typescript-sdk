use crate::{
    circuit_types::keychain::PublicSigningKey, common::types::Wallet, exports::error::WasmError,
...
pub fn handle_key_rotation(wallet: &mut Wallet, new_public_key: &str) -> Result<(), WasmError> {
    let new_pk_root = bytes_from_hex_string(new_public_key)
        .map_err(|e| WasmError::InvalidParameter(format!("Invalid public key hex: {}", e)))?;
    ...
    if current_pk_root != new_pk_root {
        ...
        let new_pk_root = PublicSigningKey::from_bytes(&new_pk_root)
            .map_err(|e| WasmError::Custom(format!("Failed to parse public key: {}", e)))?;
