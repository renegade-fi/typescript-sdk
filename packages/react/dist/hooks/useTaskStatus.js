...
import { RENEGADE_AUTH_HEADER_NAME, RENEGADE_SIG_EXPIRATION_HEADER_NAME, TASK_STATUS_ROUTE, } from "@renegade-fi/core";
import JSONBigint from "json-bigint";
import { useEffect, useRef, useState } from "react";
import useWebSocket, { ReadyState } from "react-use-websocket";
import { getSkRoot, useStatus, useWalletId } from "../index.js";
export function useTaskStatus(parameters) {
    ...
    useEffect(() => {
        if (lastMessage && taskId) {
            try {
                ...
                if (messageData.topic === TASK_STATUS_ROUTE(taskId) &&
                    messageData.event?.type === "TaskStatusUpdate" &&
                    messageData.event?.status)
                    setTask(messageData.event.status);
            }
            catch (error) {
...
//# sourceMappingURL=useTaskStatus.js.map